...
{viewerjumpto "Syntax" "did_multiplegt_old##syntax"}{...}
{viewerjumpto "Description" "did_multiplegt_old##description"}{...}
{viewerjumpto "Options" "did_multiplegt_old##options"}{...}
{viewerjumpto "Examples" "did_multiplegt_old##examples"}{...}
{viewerjumpto "Saved results" "did_multiplegt_old##saved_results"}{...}

{title:Title}

{p 4 8}
{cmd:did_multiplegt_old} {hline 2} Estimation in Difference-in-Difference (DID)
...
{p 4 8}
{cmd:did_multiplegt_old Y G T D} {ifin}
[{cmd:,}
{cmd:robust_dynamic}
{cmd:dynamic(}{it:#}{cmd:)}
{cmd:placebo(}{it:#}{cmd:)}
...
{p 4 4}
{cmd:did_multiplegt_old} estimates the effect of a treatment on an outcome, 
...
({cmd:csdid}, {cmd:didimputation}, ...), {cmd:did_multiplegt_old} can be used 
with a binary and staggered (absorbing) treatment. But unlike those other 
commands, {cmd:did_multiplegt_old} can also be used with a non-binary treatment 
...
{p 4 4}
{it: Do I have to include group- and time- fixed effects in my regression when using the did_multiplegt_old package?}
{p_end}
...
Then, you need to run {cmd:did_multiplegt_old}
...
run {cmd:did_multiplegt_old} with Cohort as the group variable;
...
{p 4 4}
{it:Is {cmd:did_multiplegt_old} compatible with the {cmd:honestdid} command?}
{p_end}

{p 4 4}
Yes, it is.
You can find an example of how to use {cmd:honestdid} after {cmd:did_multiplegt_old}
...
{cmd:did_multiplegt_old} saves the following in {cmd:e()}:
{p_end}
...
{p 4 8}
{cmd:e(cmd)}: macro equal to "did_multiplegt_old", the name of the command.
{p_end}
...
{p_end}
{p 4 8}
did_multiplegt_old lwage nr year union, placebo(1) breps(100)cluster(nr)
{p_end}
{p 4 8}
did_multiplegt_old lwage nr year union, robust_dynamic dynamic(1) placebo(1) breps(100) cluster(nr)
{p_end}
