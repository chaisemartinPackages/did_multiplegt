...
** This version: May 24, 2023
** This version of the command computes the covariance between placebos, so that the option jointtestplacebo can be included. Also improves some error messages in relation with the option placebo.

// How is the jointtestplacebo computed in the old command:
/*
...
capture program drop did_multiplegt_var_v17

program did_multiplegt_var_v17, eclass
	version 12.0
...
if "`placebo'"!=""{
	...
	bys group_XX: gen L_g_placebo_temp_XX = min(L_g_XX, F_g_XX-3)
...
scalar l_XX=min(l_XX, `=`dynamic'+1')
...
scalar l_placebo_XX=min(l_placebo_XX, `placebo')
	...
	if `dynamic'<`placebo'&`dynamic'!=0{
		scalar l_placebo_XX=min(l_XX-1, l_placebo_XX)

	}
	
	if `dynamic'==0&`placebo'!=0{
...
if "`switchers'"=="in"{
	scalar l_XX=min(`=`dynamic'+1', L_u_XX)
	if ("`placebo'"!=""){
		scalar l_placebo_XX=min(`placebo', L_placebo_u_XX)
	if `dynamic'<`placebo'&`dynamic'!=0{
		scalar l_placebo_XX=min(l_XX-1, L_placebo_u_XX)
...
if "`switchers'"=="out"{
	scalar l_XX=min(`=`dynamic'+1', L_a_XX)
		if ("`placebo'"!=""){
		scalar l_placebo_XX=min(`placebo', L_placebo_a_XX)
	if `dynamic'<`placebo'&`dynamic'!=0{
		scalar l_placebo_XX=min(l_XX-1, L_placebo_a_XX)
...
if l_XX<`=`dynamic'+1'{
	...
	di as error "The number of dynamic effects which can be estimated is at most " l_XX-1 "."
	di as error "The command will therefore try to estimate " l_XX-1 " dynamic effect(s)."
}


if l_placebo_XX<`placebo'&`dynamic'>=`placebo'{
	...
	if `dynamic'<`placebo'&`placebo'>l_placebo_XX{
		
		di as error "The number of placebo estimators requested cannot be larger than the number of dynamic effects requested."
...
forvalue i=1/`=l_placebo_XX'{	
	...
	capture drop U_Gg_var_global_pla_`i'_XX
	capture drop U_Gg_var_global_pla_`i'_2_XX
...
// 1st version of the covariances 
/*
	forvalue j=1/`=l_placebo_XX'{
		capture drop Cov_`i'_`j'_global_XX
		capture drop Cov_`i'_`j'_in_XX
		capture drop Cov_`i'_`j'_out_XX
		
		gen Cov_`i'_`j'_in_XX=0
		gen Cov_`i'_`j'_out_XX=0
	}
	*/
}	
}
...
////// Perform here the estimation, i.e. call the program did_multiplegt_var_core_v17. //

// For switchers in
if ("`switchers'"==""|"`switchers'"=="in"){
	did_multiplegt_var_core_v17 outcome_XX group_XX time_XX treatment_XX, dynamic(`dynamic') switchers_core(in) controls(`controls') placebo(`placebo') trends_nonparam(`trends_nonparam')
...
if ("`placebo'"!=""){
	forvalue i=1/`=l_placebo_u_a_XX'{	
		if N1_placebo_`i'_XX!=0{ //Display Error message if N1_placebo_`i'_XX==0
				...
				replace U_Gg_var_placebo_`i'_in_XX=U_Gg_pla_`i'_var_XX

				forvalue j=1/`=l_placebo_u_a_XX'{
					if N1_placebo_`j'_XX!=0{
					//replace Cov_`i'_`j'_in_XX= Cov_`i'_`j'_XX
				}
				}
...
// For switchers out
if ("`switchers'"==""|"`switchers'"=="out"){
	did_multiplegt_var_core_v17 outcome_XX group_XX time_XX treatment_XX, dynamic(`dynamic') switchers_core(out) controls(`controls') placebo(`placebo') trends_nonparam(`trends_nonparam')
...
if ("`placebo'"!=""){
	...
	if N0_placebo_`i'_XX!=0{
			...
			replace U_Gg_var_placebo_`i'_out_XX=U_Gg_pla_`i'_var_XX
			
		forvalue j=1/`=l_placebo_u_a_XX'{	
			if N0_placebo_`j'_XX!=0{
			//replace Cov_`i'_`j'_out_XX= Cov_`i'_`j'_XX
			}
		}
	}
	}		
...
matrix mat_res_XX = J(l_XX+l_placebo_XX+1,6,.) 
...
* Storing the results 
matrix mat_res_XX[`i',1]=DID_`i'_XX 
...
* Storing the results
matrix mat_res_XX[l_XX+1,1]=delta_XX
...
matrix mat_res_XX[l_XX+1,5]=N_effect_XX
...
// Storing the results and the number of switchers into the matrix mat_res_XX
* Storing the results 
matrix mat_res_XX[`=l_XX'+1 +`i',1]=DID_placebo_`i'_XX
...
gen U_Gg_var_global_pla_`i'_XX = U_Gg_var_placebo_`i'_in_XX * (N1_placebo_`i'_XX/(N1_placebo_`i'_XX+N0_placebo_`i'_XX)) + U_Gg_var_placebo_`i'_out_XX* (N0_placebo_`i'_XX/(N1_placebo_`i'_XX+N0_placebo_`i'_XX))

gen U_Gg_var_global_pla_`i'_2_XX = U_Gg_var_global_pla_`i'_XX^2*first_obs_by_gp_XX

sum U_Gg_var_global_pla_`i'_2_XX
scalar sum_for_var_placebo_`i'_XX=r(sum)/G_XX^2

scalar se_placebo_`i'_XX = sqrt(sum_for_var_placebo_`i'_XX)
matrix mat_res_XX[`=l_XX'+1+`i',2]=se_placebo_`i'_XX
ereturn scalar se_placebo_`i'=se_placebo_`i'_XX
	
// Lower bound of the 95% confidence interval
scalar LB_CI_placebo_`i'_XX = DID_placebo_`i'_XX - 1.96*se_placebo_`i'_XX
matrix mat_res_XX[`=l_XX'+1+`i', 3]= LB_CI_placebo_`i'_XX

// Upper bound of the 95% confidence interval
scalar UB_CI_placebo_`i'_XX = DID_placebo_`i'_XX + 1.96*se_placebo_`i'_XX
matrix mat_res_XX[`=l_XX'+1+`i',4]=UB_CI_placebo_`i'_XX
...
if ("`placebo'"!="")&l_placebo_XX>1{
	...
	forvalue i=1/`=l_placebo_XX'{
		...
		forvalue j=`=`i'+1'/`placebo'{
			...
			gen U_Gg_var_`i'_`j'_XX = U_Gg_var_global_pla_`i'_XX + U_Gg_var_global_pla_`j'_XX
...
///// Estimating the covariance between the placebos to compute the option joint_test_placebo //

// 1st version of the covariances 
/*
if "`jointtestplacebo'"!=""&l_placebo_XX>1{
...
forvalue i=1/`=l_placebo_XX'{
	...
	matrix didmgt_Placebo[`i',1]=DID_placebo_`i'_XX // DID_placebo_`i'_XX squared ?
	scalar Covar_`i'_`i'_int_XX =se_placebo_`i'_XX^2
	
forvalue j=1/`=l_placebo_XX'{
	
	//gen Cov_`i'_`j'_global_XX = Cov_`i'_`j'_in_XX * ((N1_placebo_`i'_XX+N1_placebo_`j'_XX)/(N1_placebo_`i'_XX+N0_placebo_`i'_XX+N1_placebo_`j'_XX+N0_placebo_`j'_XX)) + Cov_`i'_`j'_out_XX * ((N0_placebo_`i'_XX+N0_placebo_`j'_XX)/(N1_placebo_`i'_XX+N0_placebo_`i'_XX+N1_placebo_`j'_XX+N0_placebo_`j'_XX))
	gen Cov_`i'_`j'_global_XX = Cov_`i'_`j'_in_XX * ((N1_placebo_`i'_XX*N1_placebo_`j'_XX)/(N1_placebo_`i'_XX*N1_placebo_`j'_XX+N0_placebo_`i'_XX*N0_placebo_`j'_XX)) + Cov_`i'_`j'_out_XX * ((N0_placebo_`i'_XX*N0_placebo_`j'_XX)/(N1_placebo_`i'_XX*N1_placebo_`j'_XX+N0_placebo_`i'_XX*N0_placebo_`j'_XX))
	
	replace Cov_`i'_`j'_global_XX = Cov_`i'_`j'_global_XX * first_obs_by_gp_XX
	
	sum Cov_`i'_`j'_global_XX 
	scalar Covar_`i'_`j'_XX=r(sum)/G_XX^2
	//scalar Covar_`i'_`j'_XX=(Covar_`i'_`j'_XX-DID_placebo_`i'_XX*DID_placebo_`j'_XX)
	
	if `i'==`j'{
	matrix didmgt_Var_Placebo[`i',`j']= Covar_`i'_`i'_int_XX
	}
	else {
	matrix didmgt_Var_Placebo[`i',`j']= Covar_`i'_`j'_XX
	matrix didmgt_Var_Placebo[`j',`i']= Covar_`i'_`j'_XX
	}
}
}
...
capture program drop did_multiplegt_var_core_v17


program did_multiplegt_var_core_v17, eclass
	version 12.0
...
scalar l_u_a_XX=min(`=`dynamic'+1', L_u_XX)
	if ("`placebo'"!=""){
	scalar l_placebo_u_a_XX=min(`placebo', L_placebo_u_XX)
if `dynamic'<`placebo'&`dynamic'!=0{
		scalar l_placebo_u_a_XX=min(l_u_a_XX-1, L_placebo_u_XX)

	}
	
if `dynamic'==0&`placebo'!=0{
	scalar l_placebo_u_a_XX=0
...
scalar l_u_a_XX=min(`=`dynamic'+1', L_a_XX)
	if ("`placebo'"!=""){
	scalar l_placebo_u_a_XX=min(`placebo', L_placebo_a_XX)
if `dynamic'<`placebo'&`dynamic'!=0{
		scalar l_placebo_u_a_XX=min(l_u_a_XX-1, L_placebo_a_XX)
...
gen distance_to_switch_`i'_XX=(time_XX==F_g_XX-1+`i'&`i'<=L_g_XX&R_g_XX==increase_XX&N_gt_control_`i'_XX>0&N_gt_control_`i'_XX!=.) if diff_y_`i'_XX!=.
...
gen dummy_U_Gg`i'_XX = (`i'<=T_g_XX-1) //This dummy is also used for the placebos
...
replace U_Gg`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_`i'_XX) * N_gt_XX * [distance_to_switch_`i'_XX - (N`=increase_XX'_t_`i'_g_XX/N_gt_control_`i'_XX) * never_change_d_`i'_XX] * (time_XX>=`=`i'+1'&time_XX<=T_g_XX) * (diff_y_`i'_XX -mean_diff_y_`i'_nd_sq_t_XX)*sqrt(count_diff_y_`i'_nd_sq_t_XX/(count_diff_y_`i'_nd_sq_t_XX-1)) if never_change_d_`i'_XX==1&count_diff_y_`i'_nd_sq_t_XX>1&count_diff_y_`i'_nd_sq_t_XX!=.

replace U_Gg`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_`i'_XX) * N_gt_XX * [distance_to_switch_`i'_XX - (N`=increase_XX'_t_`i'_g_XX/N_gt_control_`i'_XX) * never_change_d_`i'_XX] * (time_XX>=`=`i'+1'&time_XX<=T_g_XX) * diff_y_`i'_XX  if never_change_d_`i'_XX==1&count_diff_y_`i'_nd_sq_t_XX==1
...
replace U_Gg`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_`i'_XX) * N_gt_XX * [distance_to_switch_`i'_XX - (N`=increase_XX'_t_`i'_g_XX/N_gt_control_`i'_XX) * never_change_d_`i'_XX] * (time_XX>=`=`i'+1'&time_XX<=T_g_XX) * (diff_y_`i'_XX - mean_diff_y_`i'_d_sq_t_XX) * sqrt(count_diff_y_`i'_d_sq_t_XX/(count_diff_y_`i'_d_sq_t_XX-1)) if distance_to_switch_`i'_XX==1&count_diff_y_`i'_d_sq_t_XX>1&count_diff_y_`i'_d_sq_t_XX!=.

replace U_Gg`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_`i'_XX) * N_gt_XX * [distance_to_switch_`i'_XX - (N`=increase_XX'_t_`i'_g_XX/N_gt_control_`i'_XX) * never_change_d_`i'_XX] * (time_XX>=`=`i'+1'&time_XX<=T_g_XX) * diff_y_`i'_XX if distance_to_switch_`i'_XX==1&count_diff_y_`i'_d_sq_t_XX==1
...
capture drop mean_diff_y_pla_`i'_nd_sq_t_XX
capture drop mean_diff_y_pla_`i'_d_sq_t_XX
capture drop U_Gg_pla_`i'_temp_var_XX
capture drop U_Gg_pla_`i'_var_XX
capture drop U_Gg_placebo_`i'_var_2_XX
capture drop count_diff_y_pla_`i'_nd_sq_t_XX
capture drop count_diff_y_pla_`i'_d_sq_t_XX
capture drop distance_to_switch_placebo_`i'_XX
capture drop never_change_d_pla_`i'_XX
...
bys group_XX : gen diff_y_placebo_`i'_XX = L`=2*`i''.outcome_XX - L`i'.outcome_XX
...
bys group_XX: gen never_change_d_pla_`i'_XX=never_change_d_`i'_XX*(diff_y_placebo_`i'_XX!=.) // Notice the similarity to the symmetric DID_l, except that we add the dummy (diff_y_placebo_`i'_XX!=.)

// N^g_t 

bys time_XX d_sq_XX `trends_nonparam': egen N_gt_control_placebo_`i'_XX=total(never_change_d_pla_`i'_XX) 
...
bys time_XX: egen N`=increase_XX'_t_placebo_`i'_XX=total(distance_to_switch_placebo_`i'_XX)
...
bys time_XX d_sq_XX `trends_nonparam': egen N`=increase_XX'_t_placebo_`i'_g_XX=total(distance_to_switch_placebo_`i'_XX)

///// Computing the mean of first or long-differences of outcomes for non-treated and for treated separately - will be useful for the computation of the variance //
bys time_XX d_sq_XX `trends_nonparam' : egen mean_diff_y_pla_`i'_nd_sq_t_XX=mean(diff_y_placebo_`i'_XX) if never_change_d_pla_`i'_XX==1&N`=increase_XX'_t_placebo_`i'_XX>0&N`=increase_XX'_t_placebo_`i'_XX!=.

bys time_XX d_sq_XX `trends_nonparam' : egen count_diff_y_pla_`i'_nd_sq_t_XX=count(diff_y_placebo_`i'_XX) if never_change_d_pla_`i'_XX==1&N`=increase_XX'_t_placebo_`i'_XX>0&N`=increase_XX'_t_placebo_`i'_XX!=.

bys time_XX d_sq_XX `trends_nonparam' : egen mean_diff_y_pla_`i'_d_sq_t_XX=mean(diff_y_placebo_`i'_XX) if distance_to_switch_placebo_`i'_XX==1 

bys time_XX d_sq_XX `trends_nonparam' : egen count_diff_y_pla_`i'_d_sq_t_XX=count(diff_y_placebo_`i'_XX) if distance_to_switch_placebo_`i'_XX==1


///// If the Placebos effect could be estimated (as there are switchers), we compute it. //

if N`=increase_XX'_placebo_`i'_XX!=0{
	gen U_Gg_placebo_`i'_temp_XX = dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_placebo_`i'_XX) * N_gt_XX * [distance_to_switch_placebo_`i'_XX - (N`=increase_XX'_t_placebo_`i'_g_XX/N_gt_control_placebo_`i'_XX) * never_change_d_pla_`i'_XX]*diff_y_placebo_`i'_XX 
	...
	**
	...
	gen count_placebo_`i'_XX_temp=(U_Gg_placebo_`i'_temp_XX!=.&U_Gg_placebo_`i'_temp_XX!=0|(U_Gg_placebo_`i'_temp_XX==0&diff_y_placebo_`i'_XX==0&(distance_to_switch_placebo_`i'_XX!=0|(N`=increase_XX'_t_placebo_`i'_g_XX!=0&never_change_d_pla_`i'_XX!=0))))
	...
	// Initializing the U_{G,g,l}_var at 0 //
	gen U_Gg_pla_`i'_temp_var_XX =0


	// Replacing U_Gg`i'_temp_var_XX by its value : the same as U_{G,g,l}, except that the average value of diff_y_`i'_XX among non-switchers is removed from diff_y_`i'_XX //
	
	replace U_Gg_pla_`i'_temp_var_XX =  dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_placebo_`i'_XX) * N_gt_XX * [distance_to_switch_placebo_`i'_XX - (N`=increase_XX'_t_placebo_`i'_g_XX/N_gt_control_placebo_`i'_XX) * never_change_d_pla_`i'_XX] * (time_XX>=`=`i'+2'&time_XX<=T_g_XX) * (diff_y_placebo_`i'_XX -mean_diff_y_pla_`i'_nd_sq_t_XX)*sqrt(count_diff_y_pla_`i'_nd_sq_t_XX/(count_diff_y_pla_`i'_nd_sq_t_XX-1)) if never_change_d_pla_`i'_XX==1&count_diff_y_pla_`i'_nd_sq_t_XX>1&count_diff_y_pla_`i'_nd_sq_t_XX!=.

	replace U_Gg_pla_`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_placebo_`i'_XX) * N_gt_XX * [distance_to_switch_placebo_`i'_XX - (N`=increase_XX'_t_placebo_`i'_g_XX/N_gt_control_placebo_`i'_XX) * never_change_d_pla_`i'_XX] * (time_XX>=`=`i'+2'&time_XX<=T_g_XX) * diff_y_placebo_`i'_XX  if never_change_d_pla_`i'_XX==1&count_diff_y_pla_`i'_nd_sq_t_XX==1
	...
	replace U_Gg_pla_`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_placebo_`i'_XX) * N_gt_XX * [distance_to_switch_placebo_`i'_XX - (N`=increase_XX'_t_placebo_`i'_g_XX/N_gt_control_placebo_`i'_XX) * never_change_d_pla_`i'_XX] * (time_XX>=`=`i'+2'&time_XX<=T_g_XX) * (diff_y_placebo_`i'_XX - mean_diff_y_pla_`i'_d_sq_t_XX) * sqrt(count_diff_y_pla_`i'_d_sq_t_XX/(count_diff_y_pla_`i'_d_sq_t_XX-1)) if distance_to_switch_placebo_`i'_XX==1&count_diff_y_pla_`i'_d_sq_t_XX>1&count_diff_y_pla_`i'_d_sq_t_XX!=.

	replace U_Gg_pla_`i'_temp_var_XX= dummy_U_Gg`i'_XX*(G_XX / N`=increase_XX'_placebo_`i'_XX) * N_gt_XX * [distance_to_switch_placebo_`i'_XX - (N`=increase_XX'_t_placebo_`i'_g_XX/N_gt_control_placebo_`i'_XX) * never_change_d_pla_`i'_XX] * (time_XX>=`=`i'+2'&time_XX<=T_g_XX) * diff_y_placebo_`i'_XX if distance_to_switch_placebo_`i'_XX==1&count_diff_y_pla_`i'_d_sq_t_XX==1

	// Summing the U_{G,g,l}s over time periods for each group //
	bys group_XX: egen U_Gg_pla_`i'_var_XX=total(U_Gg_pla_`i'_temp_var_XX)

	//End of placebos computation 
	
	// 1st version of the covariances
	//capture drop test_melitine_`i'
	//gen test_melitine_`i' = U_Gg_pla_`i'_var_XX
...
capture drop Cov_`i'_`j'_XX

gen Cov_`i'_`j'_XX = test_melitine_`i'* test_melitine_`j'
...
gen delta_D_`i'_temp_XX = N_gt_XX/N`=increase_XX'_`i'_XX*[(treatment_XX-d_sq_XX)* R_g_XX + (1-R_g_XX)*(d_sq_XX-treatment_XX)] if R_g_XX==increase_XX&F_g_XX-1<=T_g_XX-`i'&time_XX==F_g_XX-1+`i'&diff_y_`i'_XX!=.&N_gt_control_`i'_XX>0&N_gt_control_`i'_XX!=.
